...
DELETE FROM `skyfire_string` WHERE `entry` BETWEEN 177 AND 183;
INSERT INTO `skyfire_string` (`entry`, `content_default`) VALUES
(177, '|cff0099FFPhaseMgr: Report for player: %s, zoneId: %u, level: %u, team: %u, phaseupdateflag: %u|r'),
(178, '|cff663399PhaseMgr: There are no definitions defined for zoneId %u.|r'),
(179, '|cff0066FFPhaseMgr: Success (entry: %u) - added %s %u to the players phase.|r'),
(180, '|cffFF0000PhaseMgr: Condition for phase %u (entry: %u, zoneId: %u) failed.|r'),
(181, '|cffFF0000PhaseMgr: Condition for phase %u (entry: %u, zoneId: %u) has last phasemask flag. Skipped other definitions.|r'),
(182, '|cff6699FFPhaseMgr: The player gets phasemask %u through definitions, %u through phasing auras, and phase %u through custom phase.|r'),
(183, '|cff0099FFPhaseMgr: The player has phasemask %u (real: %u).|r');

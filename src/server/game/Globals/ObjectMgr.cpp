...
void ObjectMgr::LoadPhaseDefinitions()
{
    ...
    sLog->outDebug(LOG_FILTER_DATABASE_AI, ">> Loaded %u phasing definitions in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadSpellPhaseInfo()
{
    ...
    while (result->NextRow());
    sLog->outDebug(LOG_FILTER_DATABASE_AI, ">> Loaded %u spell dbc infos in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
